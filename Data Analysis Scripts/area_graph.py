...
powers = ["ipmi", "powerstat", "turbostat"]
machines = ["disk", "memory", "client", "metadata", "storage"]
finalDict = {}
        ...
        for type in types:
                for node in nodes:
                        ...
                        for i in range(1,4):
                                ...
                                print(filename)
                                val = plot_data.prep_data(fr"C:\Users\mayam\Energy_Python\{node}\{type}\{filename}", type, iotype, node, idle_considered)
